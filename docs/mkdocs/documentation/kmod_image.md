...
To avoid copying those dependencies into the kmod image, KMM mounts `/usr/lib/modules` and `/lib/modules` directories of the host into the
worker Pod's filesystem.  
By creating a symlink from `/opt/lib/modules/[kernel-version]/[symlink-name]` to
`/lib/modules/[kernel-version]`, `depmod` can use the in-tree kmods on the building node's filesystem to resolve
