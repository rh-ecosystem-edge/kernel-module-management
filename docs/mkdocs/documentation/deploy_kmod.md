...
!!! note
    DRA requires Openshift 4.21 or later. The KMM admission webhook rejects `Module` resources with `.spec.dra` on
    older clusters.
...
`config` fields matching the `DeviceClass` spec.
...
By default, Openshift auto-mounts the service account token and root CAs into DRA driver pods.
...
!!! note
    DRA requires Openshift 4.21 or later.
