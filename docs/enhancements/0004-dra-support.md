...
- **FR-5:** The admission webhook must reject creation of a Module with `spec.dra` when the cluster OpenShift version is below 4.21.
...
- **NFR-2:** DRA features require OpenShift >= 4.21. The webhook enforces this at admission time.
...
- [ ] The webhook rejects a Module with `spec.dra` on a cluster running OpenShift < 4.21.
...
- The DRA API (`k8s.io/api/resource/v1`) is stable and available on OpenShift clusters running version 4.21+.
...
- OpenShift >= 4.21 with DRA support enabled.
