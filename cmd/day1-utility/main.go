...
func customUsage() {
	fmt.Fprintf(flag.CommandLine.Output(), "Usage: %s MC_NAME MCP_NAME IMAGE_NAME KMOD_NAME IN_TREE_KMODS_TO_REMOVE WORKER_IMAGE\n\n", os.Args[0])
	...
	fmt.Fprintln(flag.CommandLine.Output(), "IN_TREE_KMODS_TO_REMOVE: in-tree kernel module names (comma-separated), that should be unloaded. If no need - pass empty string")
...
func main() {
	...
	flag.StringVar(&inTreeModulesToRemove, "in-tree-modules-to-remove", "", "in-tree kernel modules (comma-separated) that should be removed prior to loading the oot module")
	flag.StringVar(&firmwareFilesPath, "firmware-files-path", "", "path where the firmware files are located at")

	flag.Parse()
	flag.Usage = customUsage

	yaml, err := mcproducer.ProduceMachineConfig(mcName, mpName, kernelModuleImage, kernelModule, firmwareFilesPath, workerImage, strings.Split(inTreeModulesToRemove, ","))
	if err != nil {
