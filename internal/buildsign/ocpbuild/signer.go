...
//go:generate mockgen -source=signer.go -package=ocpbuild -destination=mock_signer.go Signer
...
func newSigner(client client.Client, signImage string, ocpbuildManager ocpbuildManager, scheme *runtime.Scheme) signer {
	return &signerImpl{
		client:          client,
		scheme:          scheme,
		signImage:       signImage,
...
func (s *signerImpl) makeBuildTemplate(
	ctx context.Context,
	mld *api.ModuleLoaderData,
	pushImage bool,
	...
	hash, err := s.hash(ctx, &spec, mld.Namespace, signConfig.KeySecret.Name, signConfig.CertSecret.Name)
	if err != nil {
	...
	build := &buildv1.Build{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels:       s.ocpbuildManager.ocpbuildLabels(mld.Name, mld.KernelNormalizedVersion, ocpbuildTypeSign),
			Annotations:  s.ocpbuildManager.ocpbuildAnnotations(hash),
			Finalizers:   []string{constants.GCDelayFinalizer, constants.JobEventFinalizer},
		},
		Spec: spec,
	}

	if err := controllerutil.SetControllerReference(owner, build, s.scheme); err != nil {
...
func (s *signerImpl) hash(ctx context.Context, buildSpec *buildv1.BuildSpec, namespace, keySecretName, certSecretName string) (uint64, error) {
	dataToHash := hashData{BuildSpec: buildSpec}

	var err error

	dataToHash.PublicKeyData, err = s.getSecretBytes(ctx, types.NamespacedName{Namespace: namespace, Name: certSecretName}, "cert.pem")
	if err != nil {
		return 0, fmt.Errorf("could not get cert bytes: %v", err)
	}

	dataToHash.PrivateKeyData, err = s.getSecretBytes(ctx, types.NamespacedName{Namespace: namespace, Name: keySecretName}, "key.pem")
	if err != nil {
...
func (s *signerImpl) getSecretBytes(ctx context.Context, secretObjectKey types.NamespacedName, keyName string) ([]byte, error) {
	secret := v1.Secret{}

	if err := s.client.Get(ctx, secretObjectKey, &secret); err != nil {
