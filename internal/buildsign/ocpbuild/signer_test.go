...
var _ = Describe("signer_MakeBuildTemplate", func() {
	const (
	...
	BeforeEach(func() {
		...
		s = newSigner(clnt, signerImage, mockOCPBuildManager, scheme)
		mld = api.ModuleLoaderData{
			Name:      moduleName,
			Namespace: namespace,
...
`
		annotations := map[string]string{hashAnnotation: "1404066013727235306"}
		...
		gomock.InOrder(
			...
			mockOCPBuildManager.EXPECT().ocpbuildLabels(moduleName, kernelVersion, ocpbuildTypeSign).Return(labels),
			mockOCPBuildManager.EXPECT().ocpbuildAnnotations(gomock.Any()).Return(annotations),
		)

		actual, err := s.makeBuildTemplate(ctx, &mld, true, mld.Owner)
	...
	It("should leave the build output empty if no push is configured", func() {
		...
		gomock.InOrder(
			...
			mockOCPBuildManager.EXPECT().ocpbuildLabels(moduleName, kernelVersion, ocpbuildTypeSign),
			mockOCPBuildManager.EXPECT().ocpbuildAnnotations(gomock.Any()),
		)

		actual, err := s.makeBuildTemplate(ctx, &mld, false, mld.Owner)
		Expect(err).NotTo(HaveOccurred())
