...
var _ = Describe("preflight status updates", func() {
	...
	It("preset preflight statuses", func() {
		...
		pv.Status.Modules = []v1beta2.PreflightValidationModuleStatus{
			{
				Name:         moduleName1,
				Namespace:    namespace,
				CRBaseStatus: v1beta2.CRBaseStatus{VerificationStage: v1beta1.VerificationStageBuild},
			},
			{
				Name:         moduleName2,
				Namespace:    namespace,
			...
			},
			{
				Name:         moduleName3,
				Namespace:    namespace,
				CRBaseStatus: v1beta2.CRBaseStatus{VerificationStage: v1beta1.VerificationStageImage},
			},
		}
		...
		Expect(getCRStatus(pv.Status.Modules, types.NamespacedName{Name: moduleName1, Namespace: namespace}).VerificationStage).To(Equal(v1beta1.VerificationStageBuild))
		Expect(getCRStatus(pv.Status.Modules, types.NamespacedName{Name: moduleName2, Namespace: namespace}).VerificationStage).To(Equal(v1beta1.VerificationStageBuild))
		Expect(getCRStatus(pv.Status.Modules, types.NamespacedName{Name: moduleName4, Namespace: namespace}).VerificationStage).To(Equal(v1beta1.VerificationStageImage))
		Expect(getCRStatus(pv.Status.Modules, types.NamespacedName{Name: moduleName4, Namespace: namespace}).VerificationStatus).To(Equal(v1beta1.VerificationFalse))
