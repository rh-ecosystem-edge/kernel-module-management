...
var _ = Describe("DevicePluginReconciler_setDevicePluginAsDesired", func() {
	...
	BeforeEach(func() {
		dsc = newDaemonSetCreator(scheme, operatorNamespace)
	})
	...
	DescribeTable("should add the default ServiceAccount to the device plugin if running in operator's namespace",
		func(moduleNamespace string, expectedSA string) {
			mod := kmmv1beta1.Module{
				ObjectMeta: metav1.ObjectMeta{
		...
		Entry("not running in operator's namespace", "some-namespace", ""),
		Entry("running in operator's namespace", operatorNamespace, "kmm-operator-device-plugin"),
	)
...
var _ = Describe("DevicePluginReconciler_getModuleDevicePluginDaemonSets", func() {
	...
	It("good flow, return only device plugin DSs, excluding DRA roles", func() {
