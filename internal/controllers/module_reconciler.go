...
// +kubebuilder:rbac:groups=kmm.sigs.x-k8s.io,resources=preflightvalidations/finalizers,verbs=update;patch
...
func (mr *ModuleReconciler) SetupWithManager(mgr ctrl.Manager, watchBuilds bool) error {
	...
	if watchBuilds {
		b = b.Owns(&buildv1.Build{}, builder.WithPredicates(filter.ModuleReconcileBuildPredicate()))
	}
...
func (mrh *moduleReconcilerHelper) setFinalizerAndStatus(ctx context.Context, mod *kmmv1beta1.Module) error {
	...
	// both DRA and DevicePlugin can also set finalizers, on Module, so we switch to client.MergeFromWithOptimisticLock{} in order to avoid overriding previous operations
...
func (mrh *moduleReconcilerHelper) prepareSchedulingData(ctx context.Context,
	...
	for _, node := range targetedNodes {
		...
		if mld != nil && mld.ServiceAccountName == "" && mod.Namespace == mrh.operatorNamespace {
