...
var _ = Describe("CreateOrPatch", func() {
	...
	It("should fail if we failed to create or patch", func() {
		...
		err := micAPI.CreateOrPatch(ctx, micName, micNamespace, []v1beta1.ModuleImageSpec{}, nil, "", false, nil, &kmmv1beta1.Module{})
	...
	It("should create the MIC if it doesn't exist", func() {
		...
		err := micAPI.CreateOrPatch(ctx, micName, micNamespace, images, imageRepoSecret, "", true, nil, owner)
	...
	It("should patch the MIC if it exists", func() {
		...
		err := micAPI.CreateOrPatch(ctx, micName, micNamespace, images, nil, v1.PullIfNotPresent, true, nil, owner)
