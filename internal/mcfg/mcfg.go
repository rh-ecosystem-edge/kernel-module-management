...
type MCFG interface {
	GenerateIgnition(kernelModuleImage, kernelModuleName, inTreeModuleToRemove, firmwareFilesPath, workerImage, servicePrefix string) ([]byte, error)
}
...
func (m *mcfgImpl) GenerateIgnition(kernelModuleImage, kernelModuleName, inTreeModuleToRemove, firmwareFilesPath, workerImage, servicePrefix string) ([]byte, error) {
	...
	var parsedIgnition bytes.Buffer

	if err := ignitionTemplate.Execute(&parsedIgnition, templateParams); err != nil {
		return nil, fmt.Errorf("could not render the ignition: %v", err)
	}

	var yamlIgnition map[string]interface{}
	if err := yaml.Unmarshal(parsedIgnition.Bytes(), &yamlIgnition); err != nil {
		return nil, fmt.Errorf("failed to unmarshal parsed ignition into yaml: %v", err)
	}

	jsonIgnition, err := json.Marshal(yamlIgnition)
	if err != nil {
		return nil, fmt.Errorf("failed to marshal yaml ignition into json: %v", err)
	}

	return jsonIgnition, nil
}
