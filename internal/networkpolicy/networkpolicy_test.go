...
var _ = Describe("NetworkPolicy", func() {
	...
	Describe("BuildSignPodsNetworkPolicy", func() {
		It("should return correct NetworkPolicy for build and sign pods", func() {
			...
			Expect(result.Spec.PolicyTypes).To(ContainElement(networkingv1.PolicyTypeEgress))
			Expect(result.Spec.PolicyTypes).NotTo(ContainElement(networkingv1.PolicyTypeIngress))

			Expect(result.Spec.Egress).To(HaveLen(1))
			Expect(result.Spec.Egress[0]).To(Equal(networkingv1.NetworkPolicyEgressRule{}))
