...
func ProduceMachineConfig(machineConfigName,
	...
	jsonIgnition, err := mcfgAPI.GenerateIgnition(kernelModuleImage, kernelModuleName, inTreeModuleToRemove, firmwareFilesPath, workerImageToUse, machineConfigName)
	if err != nil {
		return "", fmt.Errorf("failed to create ignition string: %v", err)
	}

	yamlIgnition, err := yaml.JSONToYAML(jsonIgnition)
	if err != nil {
		return "", fmt.Errorf("failed to convert json ignition to yaml: %v", err)
	}

	templateParams := map[string]any{
		"Ignition":             string(yamlIgnition),
